...
public class SelectWinnerCardUseCase {
   ...
   /* public Mono<Map<String, CardInGame>> selectWinnerCard(String boardId, Board board){
        var winnerCard= board.getCardsInGame().values().stream().max(Comparator.comparing(o -> o.getCard().getPower())).orElseThrow();
       var winnerRoundId = board.getCardsInGame().entrySet().stream().filter(stringCardInGameEntry ->
                       stringCardInGameEntry.getValue()==winnerCard
               ).map(Map.Entry::getKey).reduce((s, s2) -> s+s2).orElseThrow();
